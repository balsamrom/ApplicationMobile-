...
class VetProfileEditorScreen extends StatefulWidget {
  ...
  const VetProfileEditorScreen({Key? key, required this.vet}) : super(key: key);
...
class _VetProfileEditorScreenState extends State<VetProfileEditorScreen> {
  ...
  void initState() {
    super.initState();
    final owner = widget.vet.owner;
    final cabinet = widget.vet.cabinet;
    ...
    _specialtyController = TextEditingController(text: widget.vet.specialty ?? '');
    _profileImagePath = owner.photoPath;

    _addressController = TextEditingController(text: cabinet?.address ?? '');
    _latitudeController = TextEditingController(text: cabinet?.latitude?.toString() ?? '');
    _longitudeController = TextEditingController(text: cabinet?.longitude?.toString() ?? '');
  }
  ...
  Future<void> _saveProfile() async {
    if (_formKey.currentState!.validate()) {
      ...
      await DatabaseHelper.instance.updateVeterinaryProfile(ownerData, widget.vet.owner.id!);
      ...
      await DatabaseHelper.instance.saveCabinet(cabinet);

...
class PatientDetailScreen extends StatefulWidget {
  ...
  const PatientDetailScreen({Key? key, required this.patient, required this.vetId}) : super(key: key);
  ...
  Widget _buildPetsAndHistoryList() {
    return FutureBuilder<List<dynamic>>(
      ...
      builder: (context, snapshot) {
        ...
        final allVetAppointments = snapshot.data![1] as List<VeterinaryAppointment>;
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final petHistory = allVetAppointments.where((a) => a.petId == pet.id).toList();
            ...
            return Card(
              ...
              child: ExpansionTile(
                ...
                subtitle: Text('${pet.species} - ${pet.breed ?? 'Race inconnue'}'),
                children: [
                  if (petHistory.isEmpty)
                    const ListTile(
                        title: Text('Aucun historique de consultation avec vous pour cet animal.')
                    )
                  else
                    ...petHistory.map((appointment) {
                      return Card(
                        ...
                        child: ListTile(
                          ...
                          trailing: const Icon(Icons.edit_note, color: accentOrange),
                          onTap: () async {
                            final vets = await DatabaseHelper.instance.getVeterinarians();
                            final vet = vets.firstWhereOrNull((v) => v.owner.id == widget.vetId);

                            if (vet == null) {
                              if (mounted) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    content: Text('Erreur: Le profil de ce vétérinaire est introuvable.'),
